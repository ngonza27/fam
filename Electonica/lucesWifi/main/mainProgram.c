...
char *jsonParse(char *msg){;
    ...
    action[(actionEnd-actionStart)+1]='\0';
     //codigo machete arreglar mas adelante
...
void app_main(){
    ...
    //llama a la funcion de arriba initialise_wifi localizada en smartconfig_main.c
    initialise_wifi();

    /* crea un task, que es una funcion localizada en awsiot.c y le asigna un entorno de ejecucion, pueden 
    haber varias task creados cada uno independiente del otro pero solo un se ejecuta a la vez  el rtos 
    se encarga del scheduling de los tasks. CONSUMEN BUENA RAM
    */
    xTaskCreatePinnedToCore(&aws_iot_task, "aws_iot_task", 9216, NULL, 5, NULL, 1);

    // en esta zona se configura todo lo de pine GPIO 
    gpio_config_t io_conf;
    //abilita o desabilita el modo de interrupcion
    io_conf.intr_type= GPIO_PIN_INTR_DISABLE;
    //modo del pin lectura o escitura
    io_conf.mode=GPIO_MODE_DEF_OUTPUT;
    //asigna el pin que elegimos en la parte d arriba a la configuracion
